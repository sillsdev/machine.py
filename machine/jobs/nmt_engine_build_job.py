...
class NmtEngineBuildJob(TranslationEngineBuildJob):
    ...
    def _align(
        ...
        pretranslations = list(pretranslations)

        parallel_pretranslation_corpus = DictionaryTextCorpus(
            MemoryText("pt", [TextRow("pt", i, pi[0].split()) for i, pi in enumerate(pretranslations)])
        ).align_rows(
            DictionaryTextCorpus(
                MemoryText(
                    "pt", [TextRow("pt", i, pi[1]["translation"].split()) for i, pi in enumerate(pretranslations)]
                )
            )
        )
        ...
        with (
            ...
            index = 0
            for pt_batch in batch(zip((pi[1] for pi in pretranslations), rows, strict=True), batch_size):
                if check_canceled is not None:
                    check_canceled()
