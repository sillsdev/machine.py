...
- Every corpus class streams: a transform or new corpus yields from its source
  inside `with ... get_rows() as rows:` rather than collecting rows into a list.
  A whole-Bible corpus, or several of them in a parallel corpus, must not have
  to fit in memory. The dictionary-backed corpora are the exception: they wrap
