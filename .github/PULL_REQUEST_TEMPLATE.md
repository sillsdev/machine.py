...
CI collects coverage and runs the full OS and Python matrix; local_check.sh does
neither, so a local run is not CI-equivalent. -->
...
<!-- Fixes #N only for a real GitHub issue. For ported work, link the source
machine PR. Delete if neither applies. -->
