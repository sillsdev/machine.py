...
def test_extract_scripture_corpus() -> None:
    corpus = ParatextTextCorpus(USFM_TEST_PROJECT_PATH)
    ...
    assert orig_vref.exact_equals(VerseRef.from_string("LEV 14:56", ORIGINAL_VERSIFICATION))
    assert corpus_vref is not None and corpus_vref.exact_equals(VerseRef.from_string("LEV 14:55", corpus.versification))
