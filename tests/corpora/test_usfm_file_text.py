...
def test_get_rows_sentence_start() -> None:
    ...
    assert rows[3].text == "Chapter one with odd whitespace, verse four,"
...
def test_get_rows_include_markers() -> None:
    ...
    assert scripture_ref(rows[1]) == ScriptureRef.parse("MAT 1:2", corpus.versification)
    assert rows[1].text == "\\bd C\\bd*hapter one, \\li2 verse\\f + \\fr 1:2: \\ft This is a footnote for v2.\\f* two."
...
def test_get_rows_include_markers_all_text() -> None:
    ...
    assert rows[9].text == "\\bd C\\bd*hapter one, \\li2 verse\\f + \\fr 1:2: \\ft This is a footnote for v2.\\f* two."
