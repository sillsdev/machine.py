...
def test_get_segments_nonempty_text() -> None:
    with DblBundleTestEnvironment() as env:
        ...
        assert (
            ...
            == "Jesus was born in Bethlehem town in Judea province during the time [MTY] that King Herod the Great ruled there. Some time after Jesus was born, some men who studied the stars and who lived in a country east of Judea came to Jerusalem city."
        )
...
def test_get_segments_sentence_start() -> None:
    with DblBundleTestEnvironment() as env:
        ...
        assert (
            ...
            == "to the town called Nazareth to live there. The result was that what had been said by the ancient prophets {what the ancient prophets had said} about the Messiah, that he would be called {people would call him} a Nazareth-man, was fulfilled {came true}."
        )
